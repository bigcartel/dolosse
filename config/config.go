...
Simple and efficient MySQL binlog to ClickHouse change data replication.

Dolosse supports:
- (obviously) Replicating MySQL binlog change data to ClickHouse as separate rows with details about what columns were updated with each event.
...
- Only replicating the mysql columns that are also present in the destination ClickHouse table and deduplicating row data.
	Any events which would result in a duplicate given the subset of columns in the destination table will be ignored.
- Pulling the maximum amount of binlog data available, and dumping only data not already present in the binlog.
- Doing a full table dump after completing binlog replication to ensure all data is present in ClickHouse.
- Deduplicating binlog data such that you can rewind the syncer or re-run a database dump at any time without fear of writing duplicate data.
	If you add a new table in MySQL and later decide you want to add that table to ClickHouse You can add it and the restart dolosse with the
	--rewind flag and it will pull in all change data present in the binlog for that table while ignoring any data that was already dumped for
	existing tables.
